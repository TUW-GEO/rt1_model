...
class TestBasicPlotting(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.SRFnames = [key for key, val in surface.__dict__.items() if
                 (isinstance(val, type) and issubclass(val, surface._Surface)
                  and not key.startswith("_") and not key in ["LinCombSRF"])]

        self.Vnames = [key for key, val in volume.__dict__.items() if
                 (isinstance(val, type) and issubclass(val, volume._Volume)
                  and not key.startswith("_") and not key in ["LinCombV"])]
    ...
    def test_surface_init(self):
        ...
        self.assertTrue(all(i in choices for i in self.SRFnames),
        ...
        for name, params in choices.items():
            ...
            self.assertTrue(
                all((key in init_dict and init_dict.get(key, "nope") == val)
            ...
            for key, val in params.items():
                self.assertTrue(getattr(SRF, key) == val,
    ...
    def test_volume_init(self):
        ...
        self.assertTrue(all(i in choices for i in self.Vnames),
        ...
        for name, params in choices.items():
            ...
            for key, val in params.items():
                self.assertTrue(getattr(V, key) == val,
